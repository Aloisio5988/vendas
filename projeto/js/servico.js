$(document).ready(function(){
  ...
  janela.click(function(){
    ...
    $.post('ajax/servico.php', {acao: 'form_cad'}, function(retorno){
    ...
    $("#myModal").on("submit", 'form[name="form_cad"]', function(){
        ...
        $.ajax({
            url: 'ajax/controller_servico.php',
            type: 'POST',
            ...
            beforeSend: function(){
             ...
             success : function(retorno){
                ...
                if (retorno === 'cadastrou') {
                    ...
                    msg('Serviço cadastrado com sucesso', 'sucesso');
                    listarAdmin('ajax/servico.php', 'listar_admin', true);
                   });
                    
                }else{
                    msg('Erro ao cadastrar servico', 'erro');
                } 
...
$('.table').on("click", '#btn_edit', function(){
   var id = $(this).attr('data-id');
    $.post('ajax/servico.php', {acao: 'form_edit', id: id}, function(retorno){
        $('#myModal').modal({backdrop: 'static'});
       conteudo.html(retorno);
    });
      return false; 
  ...
  $('#myModal').on("submit", 'form[name="form_edit"]', function(){
         ...
         success: function(retorno){
           if(retorno === 'atualizou'){     
             dados.fadeOut('slow', function(){
                 msg('servico atualizado', 'sucesso');
                 listarAdmin('ajax/servico.php', 'listar_admin', true);      
             });
           }else{
               msg('Você não alterou nenhum dados do servico', 'alerta');
    ...
    $('.table').on('click', '#btn_excluir', function(){
        ...
        if (confirm("Você deseja realmente excluir este servico")){
            $.post('ajax/controller_servico.php', {acao : 'excluir', id: id}, function(retorno){
                if (retorno === 'deletou'){
                    alert('Deletado');
                    listarAdmin('ajax/servico.php', 'listar_admin', true); 
                }else {
...
// realiza a listagem via jquery    
    listarAdmin('ajax/servico.php', 'listar_admin');             
