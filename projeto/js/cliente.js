$(document).ready(function(){
  ...
  janela.click(function(){
    ...
    $.post('ajax/cliente.php', {acao: 'form_cad'}, function(retorno){
    ...
    $("#myModal").on("submit", 'form[name="form_cad"]', function(){
        ...
        $.ajax({
            url: 'ajax/controller_cliente.php',
            type: 'POST',
            ...
            beforeSend: function(){
             ...
             success : function(retorno){
                ...
                if (retorno === 'cadastrou') {
                    ...
                    listarAdmin('ajax/cliente.php', 'listar_admin', true);
                   });
                    
                }else{
                    msg('Erro ao cadastrar Cliente', 'erro');
                } 
...
$('.table').on("click", '#btn_edit', function(){
   var id = $(this).attr('data-id');
    $.post('ajax/cliente.php', {acao: 'form_edit', id: id}, function(retorno){
        $('#myModal').modal({backdrop: 'static'});
       conteudo.html(retorno);
    });
      return false; 
  ...
  $('#myModal').on("submit", 'form[name="form_edit"]', function(){
         ...
         success: function(retorno){
           if(retorno === 'atualizou'){     
             dados.fadeOut('slow', function(){
                 msg('Cliente atualizado', 'sucesso');
                 listarAdmin('ajax/cliente.php', 'listar_admin', true);      
             });
           }else{
    ...
    $('.table').on('click', '#btn_excluir', function(){
        ...
        if (confirm("Você deseja realmente excluir este Cliente")){
            $.post('ajax/controller_cliente.php', {acao : 'excluir', id: id}, function(retorno){
                if (retorno === 'deletou'){
                    alert('cadastrou');
                    listarAdmin('ajax/cliente.php', 'listar_admin', true); 
                }else {
...
// realiza a listagem via jquery    
    listarAdmin('ajax/cliente.php', 'listar_admin');             
